...
The response includes `status`, `summary`, structured `result`, `goals`, `goalProgress`, `questChanges`, `inventoryChanges`, `actionReports`, `budget`, `social`, and persisted rolling-window `usage`. Use `summary` directly when possible; if reporting in your own words, include useful `social` details such as nearby players/agents and public chat that happened during the run.
