...
- For "start fishing", "go fishing", or "fish for onchain goodies", prefer the dedicated `/agent-fishing` tool with `operation: "start"`. If the dedicated tool is unavailable, use `behaviorScheme: "fishing"` with `/agent-command`, or use the manual `/agent-action` `fish` loop for one live cast/reel/loot cycle. Do not translate fishing to `farmer`; that profile farms safe targets. Include the `fishing` recap when present, especially named catches, fish sales/points, NFT catch names/status, wallet-action needs, daily remaining values, and reset time.
- If fishing returns `status: "wallet_action_required"` with `walletActionRequired.action: "claim_fishing_nft"`, claim it immediately from the Bankr wallet: send the provided `walletActionRequired.transaction`, then call `/agent-fishing` with `operation: "submit_claim_tx"`, `catchId`, and the submitted tx hash, or use `/agent-action` with `action: "submit_fishing_nft_claim_tx"`. Do not keep casting while a claim voucher is pending.
- Use `/agent-fishing` with `operation: "sell_fish"` to sell offchain fish for Season points and `operation: "refresh"` after NFT claim or Mint Club wallet actions.
