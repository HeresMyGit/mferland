...
- For X timeline or chat requests like "play for 5 minutes", "do next quest", "farm rabbits", "fish for onchain goodies", or "train DPS", use `/agent-command` and return its recap. Include the `fishing` recap when present, especially named catches, fish sales/points, NFT catch names/status, wallet-action needs, daily remaining values, and reset time.
