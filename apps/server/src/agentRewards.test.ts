...
test("agent season reward multiplier defaults to a nerf", () => {
  assert.equal(DEFAULT_AGENT_SEASON0_POINT_MULTIPLIER, 0.25);
  assert.equal(normalizeAgentSeason0PointMultiplier(""), 0.25);
  assert.equal(adjustSeason0QuestPointsForAgent(20, true), 5);
});
...
test("agent season reward multiplier clamps and preserves a positive reward when enabled", () => {
  ...
  assert.equal(adjustSeason0QuestPointsForAgent(0, true, 0.25), 0);
  assert.equal(adjustSeason0QuestPointsForAgent(5, true, 0.25), 1);
...
test("agent season reward note labels adjusted points", () => {
  assert.equal(formatRewardMultiplier(0.25), "0.25");
