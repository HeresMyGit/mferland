...
export function buildAgentCatalog() {
  return {
    ...
    controls: {
      ...
      combat: ["combatAction"],
      lootAndItems: ["lootCorpse", "equipItem", "unequipItem", "useItem", "sellTrashItems", "startFishing", "reelFishing", "cancelFishing", "sellFishingItems", "purchaseFishingSupply"],
    ...
    menus: {
      ...
      fishing: {
        ...
        observes: ["self.fishingState", "self.fishingBobber", "lootWindow.source=fishing", "inventory.fishing", "activeBuffs", "fishingResult", "fishingSupplyPurchaseResult", "fishingVendorSellResult", "season0Points", "season0DailyPoints"],
        controls: ["startFishing", "reelFishing", "lootCorpse", "cancelFishing", "purchaseFishingSupply", "sellFishingItems"],
        paidControlRequiresPaymentProof: true,
        note: `Fish through normal room messages. Cast near ${FISHING_ZONE.name}, wait for the bite state, then reel. Catches open a fishing lootWindow; collect it with lootCorpse using that lootWindow npcId/source id. Motherfisher sells bucket of old chum after fishin-lesson with an MFERGPT burn proof. Fish sales happen at fish monger after lost-fishing-shoes; the server applies Season 0 caps and declared-agent bundle rules.`,
      },
      hotbar: {
