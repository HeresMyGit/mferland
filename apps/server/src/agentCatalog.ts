...
export function buildAgentCatalog() {
  return {
    ...
    controls: {
      ...
      combat: ["combatAction"],
      lootAndItems: ["lootCorpse", "equipItem", "unequipItem", "useItem", "sellTrashItems", "startFishing", "reelFishing", "cancelFishing", "sellFishingItems"],
    ...
    menus: {
      ...
      fishing: {
        ...
        observes: ["self.fishingState", "self.fishingBobber", "lootWindow.source=fishing", "inventory.fishing", "fishingResult", "fishingVendorSellResult", "season0Points", "season0DailyPoints"],
        controls: ["startFishing", "reelFishing", "lootCorpse", "cancelFishing", "sellFishingItems"],
        paidControlRequiresPaymentProof: false,
        note: `Fish through normal room messages. Cast near ${FISHING_ZONE.name}, wait for the bite state, then reel. Catches open a fishing lootWindow; collect it with lootCorpse using that lootWindow npcId/source id. Fish sales are free room messages; the server applies Season 0 caps and declared-agent bundle rules.`,
      },
      hotbar: {
    ...
    fishing: {
      ...
      controls: {
        ...
        sellOne: { message: "sellFishingItems", shape: { itemId: "fishing catch item id", quantity: 1 } },
        sellStack: { message: "sellFishingItems", shape: { itemId: "fishing catch item id", quantity: "integer 1..999" } },
      ...
      agentPayoutNote: `Declared agents need each fish bundle size multiplied by ${FISHING_AGENT_BUNDLE_MULTIPLIER}. Zero-point junk still sells for 0 points and can be cleared.`,
      itemIds: FISHING_CATCH_ITEM_IDS,
      items: FISHING_CATCH_ITEM_IDS.map((itemId) => {
