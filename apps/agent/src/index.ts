...
function readConfig(): AgentConfig {
  const { values } = parseArgs({
    options: {
      ...
      "llm-objective": {
        type: "string",
        default: process.env.AGENT_LLM_OBJECTIVE ?? "Register or continue a wallet character. If wallet tools are configured, swap ETH to MFERGPT if needed, buy one useful potion-shop item from potion-mfer by burning MFERGPT, sell looted trash to trash-mfer when safe, then progress quests like a normal human player and cooperate with visible players.",
      },
