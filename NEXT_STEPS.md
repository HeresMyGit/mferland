...
- [x] Wire the Season 0 pass into local in-game purchase and token-distribution eligibility. Completed 2026-05-04 with pass ETH/`$mfergpt` mint UI, browser smoke coverage, and pass-gated `season-export`.
...
- [x] Document the Season Points redemption strategy and onchain-vault path. Completed in [docs/crypto/season-gold-redemption.md](docs/crypto/season-gold-redemption.md).
- [x] Add visible Season Points in the HUD or character profile and a support export that converts approved pass-gated points into proposed `$mfergpt` payouts under a fixed pool and per-wallet cap. Completed with Character panel Season Points totals and `npm run support:admin -- season-payout-export`.
